...
def test_performance_vs_nonmarkovian():
    ...
    for size in sizes:
        ...
        # We need to use a custom approach to get all documents since get_future_documents has max_hops=5
        # Instead, we'll use the graph's get_forward_nodes directly with higher max_hops
        future_docs = []
        current_doc = "doc_0"
        max_hops = size + 10  # Ensure we can reach all documents
        
        # Use the graph directly to get all reachable nodes
        all_reachable = api.graph.get_forward_nodes(current_doc, time_window_days=size*2 + 10, max_hops=max_hops, max_results=size*2)
...
def test_comparison_summary():
    ...
    for size in sizes:
        ...
        # Non-Markovian
        start = time.time()
        # Use the graph directly to get all reachable nodes with higher max_hops
        all_reachable = api.graph.get_forward_nodes("doc_0", time_window_days=size+10, max_hops=size+10, max_results=size*2)
        non_markovian_docs = [api._node_to_data(api.graph.nodes[nid]) for nid in all_reachable]
